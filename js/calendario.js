...
var settings = {
    ...
    NavLocation: '#calendario',          //(string - element) where to display navigation, if not in default position.
...
// esta parte fue hecha 100% por mi, ya que la libreria no traia un form para agregar fechas dinamicamente, la pagina de la libreria traia un ejemplo de como hardcodear fechas:
...
// entonces yo trabaje a partir de es para poder generarlos con el form
...
var eventos = JSON.parse(localStorage.getItem("fechas"))
    ...
    function agregarFecha() {
        ...
        eventos.push(new Info(desc, dia, mes, ano))
        localStorage.setItem('fechas', JSON.stringify(eventos));
    }
    let events = [];
    for (el of eventos) {
        events.push({
            'Date': new Date(el.ano, el.mes - 1, el.dia),
            'Title': el.desc,
            'Link': function () {
                console.log('Anotado!');
            }
